...
<body class="flex h-screen overflow-hidden bg-[#f9f9f9]">
    <div class="w-[45%] bg-[#b829e3] flex flex-col justify-center items-end pr-40 relative z-20 shadow-[10px_0_30px_rgba(184,41,227,0.05)]" style="border-top-right-radius: 50% 50%; border-bottom-right-radius: 50% 50%;">
        ...
        <button onclick="if(document.referrer) { window.history.back(); } else { window.location.href = '/'; }" class="absolute top-10 left-10 text-white font-semibold text-xl flex items-center gap-2 hover:-translate-x-2 transition-transform duration-300">
    ...
    <div class="flex-1 flex justify-center items-center relative z-10 pl-4">
        <div class="bg-white p-12 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] w-[480px] hover:shadow-[0_15px_50px_rgba(184,41,227,0.15)] transition-shadow duration-500">
            ...
            <?php if(isset($_SESSION['error'])): ?>
                <div class="bg-red-50 border border-red-200 text-red-500 text-sm text-center mb-6 py-3 px-4 rounded-lg font-medium shadow-sm">
            ...
            <?php if(isset($_SESSION['success'])): ?>
                <div class="bg-green-50 border border-green-200 text-green-600 text-sm text-center mb-6 py-3 px-4 rounded-lg font-medium shadow-sm">
            ...
            <form action="/login" method="POST" class="space-y-6">
                ...
                <div class="group">
                    <label class="block text-gray-400 text-sm mb-1.5 ml-2 font-medium group-focus-within:text-[#b829e3] transition-colors duration-300">Username / Email</label>
                    <input type="text" name="username" placeholder="Input your Username or E-mail" required class="w-full bg-gray-100/80 rounded-full px-6 py-4 outline-none focus:bg-white focus:ring-4 focus:ring-purple-200 focus:border-[#b829e3] border border-transparent transition-all duration-300 shadow-sm">
                </div>
                <div class="group">
            ...
            <p class="text-center mt-8 text-gray-400 font-medium">Don't Have any account? <a href="/register" class="text-gray-600 hover:text-[#b829e3] underline transition-colors duration-300">Create Account</a></p>
        </div>
    </div>
</body>
</html>