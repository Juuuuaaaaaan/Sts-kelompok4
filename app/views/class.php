...
try {
    // 1. Ambil data kelas yang DIBUAT OLEH USER INI (Untuk Host Game)
    $query_my_classes = "SELECT * FROM classes WHERE created_by = :username";
    $stmt_my_classes = $pdo->prepare($query_my_classes);
    $stmt_my_classes->execute(['username' => $username]);
    $my_classes = $stmt_my_classes->fetchAll(PDO::FETCH_ASSOC);

    // 2. Ambil data HISTORY kelas yang sudah dimainkan
    // Menggunakan JOIN agar jika kelasnya dihapus oleh pembuatnya, otomatis tidak muncul di history
...
<body class="bg-[#f3f4f6] min-h-screen flex flex-col relative">
   ...
   <?php $current_page = basename($_SERVER['PHP_SELF']); ?>
    ...
    <?php if (in_array($msg, ['deleted', 'history_deleted', 'history_cleared'])) : ?>
        ...
        <script>
            // Tunggu 2.5 detik, lalu mulai animasi fade out dan slide up
            setTimeout(() => {
                const toast = document.getElementById('toastNotification');
                if (toast) {
                    // Mengubah class Tailwind untuk memicu animasi
                    toast.classList.remove('opacity-100', 'translate-y-0');
                    toast.classList.add('opacity-0', '-translate-y-10');
                    
                    // Menghapus elemen dari layar sepenuhnya setelah animasi selesai (1 detik)
